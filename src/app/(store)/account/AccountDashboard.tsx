...
export default function AccountDashboard({ customer, orders }: AccountDashboardProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 md:gap-12 items-start">
      ...
      {/* Sidebar Navigation */}
      <div className="lg:col-span-1 space-y-3 flex flex-row lg:flex-col overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 border-b lg:border-b-0 border-onyx/5 hide-scrollbar gap-2 lg:gap-0">
        {[
      ...
      <div className="lg:col-span-3 min-h-[400px]">
        {/* Loading Overlay */}
        {isPending && (
          <div className="fixed inset-0 bg-onyx/10 backdrop-blur-[1px] z-50 pointer-events-none flex items-center justify-center">
            <div className="bg-neutral-soft/80 p-4 rounded-full shadow-lg">
              ...
              {orders.length > 0 ? (
                <div className="space-y-4">
                  {orders.slice(0, 2).map((order) => (
                    <Card key={order.orderId || order._id} variant="outline" className="bg-neutral-soft border-onyx/5 hover:border-onyx transition-colors duration-300">
                      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-onyx/40 font-medium">
                        ...
                        <div className="flex items-center justify-between md:justify-end gap-8">
                          ...
                          <button 
                            ...
                            className="bg-onyx hover:bg-zinc-800 text-white font-black uppercase tracking-widest text-[9px] px-6 py-3 rounded-lg cursor-pointer transition-colors"
                          >
                            Track Order
                          </button>
                        </div>
                  ...
                  {orders.length > 2 && (
                    <button 
                      onClick={() => handleTabChange('orders')}
                      className="w-full text-center text-xs font-black uppercase tracking-widest py-4 border border-dashed border-onyx/10 rounded-xl hover:border-onyx transition-colors cursor-pointer text-onyx/60"
                    >
                ...
                <Card variant="outline" className="bg-neutral-soft border-onyx/5 text-center py-16">
                  <div className="flex justify-center mb-4">
                    <svg className="w-10 h-10 text-onyx/30" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                  </div>
                  <p className="text-sm font-bold text-onyx/40">You haven't placed any orders yet.</p>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-8 animate-fade-in">
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tight text-onyx">Order History & Tracking</h2>
              <p className="text-onyx/60 font-medium mt-1">Track active orders and view past purchase receipts.</p>
            </div>
            ...
            {orders.length > 0 ? (
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <Card 
                      ...
                      variant="outline" 
                      padding="p-0" 
                      className={`overflow-hidden bg-neutral-soft shadow-sm transition-all duration-300 border-onyx/5 ${
                        isExpanded ? 'ring-2 ring-onyx border-transparent' : 'hover:border-onyx/20'
                      }`}
                    >
                      {/* Order Header / Collapsible Trigger */}
                      <div 
                        onClick={() => setExpandedOrderId(isExpanded ? null : order._id)}
                        className="px-8 py-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 cursor-pointer hover:bg-bone/50"
                      >
                        <div className="flex flex-wrap gap-x-8 gap-y-2">
                          <div>
                            <p className="text-[9px] uppercase tracking-widest font-black text-onyx/40">Order ID</p>
                            <p className="text-xs font-mono font-black text-onyx">
                              #{order.orderId || order._id.slice(-8).toUpperCase()}
                            </p>
                          </div>
                          <div>
                            <p className="text-[9px] uppercase tracking-widest font-black text-onyx/40">Date</p>
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                          <div className="flex gap-2">
                            ...
                            <Badge variant="neutral" className="h-7 px-3 font-black uppercase text-[8px] tracking-widest bg-onyx/5 border-none flex items-center gap-1 text-onyx/80">
                      ...
                      {isExpanded && (
                        <div className="border-t border-onyx/5 p-8 space-y-10 bg-bone/20">
                          {/* Order Timeline Tracking */}
                          <div className="space-y-4">
                            <h4 className="text-[10px] uppercase tracking-[0.25em] font-black text-onyx/40">Delivery Status</h4>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                            ...
                            <div className="lg:col-span-7 space-y-6">
                              <h4 className="text-[10px] uppercase tracking-[0.25em] font-black text-onyx/40">Tracking Log</h4>
                              {order.trackingId ? (
                                <div className="space-y-6">
                                  <div className="bg-neutral-soft border border-onyx/5 p-5 rounded-xl flex items-center gap-4">
                                    ...
                                    <div>
                                      <p className="text-[8px] uppercase tracking-[0.2em] font-black text-onyx/40">Carrier Tracking ID</p>
                                      <p className="text-xs font-mono font-black text-onyx tracking-widest uppercase">{order.trackingId}</p>
                                    </div>
                                  </div>
                                  ...
                                  {order.trackingUpdates && order.trackingUpdates.length > 0 ? (
                                    ...
                                    <p className="text-xs text-onyx/40 italic font-medium pl-2">No tracking timeline updates logged yet.</p>
                                  )}
                                </div>
                              ) : (
                                <div className="bg-neutral-soft border border-dashed border-onyx/10 p-6 rounded-xl text-center">
                                  <p className="text-xs text-onyx/40 font-medium">
                            ...
                            {/* Order items */}
                            <div className="lg:col-span-5 space-y-6 lg:border-l border-onyx/5 lg:pl-10">
                              <h4 className="text-[10px] uppercase tracking-[0.25em] font-black text-onyx/40">Order Items</h4>
                              <div className="space-y-4">
                                {order.items?.map((item: any, idx: number) => (
                                  <div key={idx} className="flex gap-4 items-center">
                                    <div className="w-14 h-18 bg-bone flex-shrink-0 overflow-hidden rounded-lg border border-onyx/5 relative">
                                      {item.productImage ? (
                                        <img 
                                        ...
                                        <div className="w-full h-full bg-onyx/5 flex items-center justify-center">
                                          <svg className="w-5 h-5 text-onyx/40" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                                        </div>
                                      )}
                                    ...
                                    <div className="flex-grow min-w-0">
                                      <p className="text-xs font-black text-onyx uppercase truncate leading-tight">
                                      ...
                                      {(item.color || item.size) && (
                                        <div className="flex gap-2 mt-1.5">
                                          {item.color && (
                                            <span className="text-[7px] uppercase tracking-widest font-black bg-onyx/5 text-onyx/70 px-1.5 py-0.5 rounded">
                                              COLOR: {item.color}
                                            </span>
                                          )}
                                          {item.size && (
                              ...
                              <div className="border-t border-onyx/5 pt-4 mt-6 space-y-2">
                                <div className="flex justify-between text-[10px] uppercase font-black tracking-widest text-onyx/40">
                                ...
                                <p className="text-[11px] text-onyx/70 font-medium leading-relaxed bg-neutral-soft border border-onyx/5 p-4 rounded-lg">
              ...
              <Card variant="outline" className="bg-neutral-soft border-onyx/5 text-center py-24">
                <div className="flex justify-center mb-6">
                  <svg className="w-12 h-12 text-onyx/30" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0l-3.586 3.586a2 2 0 01-2.828 0L9 13" /></svg>
                </div>
                <h3 className="text-lg font-black uppercase mb-2">No Orders Placed</h3>
                <p className="text-onyx/40 text-xs font-medium max-w-xs mx-auto leading-relaxed">
        ...
        {activeTab === 'addresses' && (
          <div className="space-y-10 animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-2xl font-black uppercase tracking-tight text-onyx">Address Book</h2>
                <p className="text-onyx/60 font-medium mt-1">Manage multiple delivery destinations and default shipping address.</p>
              </div>
              {!isAddingAddress && (
                <Button 
            ...
            {isAddingAddress && (
              <Card variant="outline" className="bg-neutral-soft border-onyx/5 shadow-sm p-8 md:p-12 animate-fade-in border-2 border-onyx">
                <div className="flex justify-between items-center border-b border-onyx/5 pb-4 mb-8">
                  <h3 className="text-lg font-black uppercase">Add New Address</h3>
                  <button 
                    onClick={() => setIsAddingAddress(false)}
                    className="text-xs uppercase tracking-widest font-black text-onyx/40 hover:text-onyx cursor-pointer border-none bg-transparent"
                  >
                    Cancel
                  </button>
                </div>
                  ...
                  <div className="flex items-center gap-3 py-2 cursor-pointer group">
                    <div 
                      ...
                      className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                        addressForm.isDefault ? 'bg-onyx border-onyx text-white' : 'border-onyx/20 group-hover:border-onyx'
                      }`}
                    >
                    ...
                    <span 
                      onClick={() => setAddressForm({...addressForm, isDefault: !addressForm.isDefault})}
                      className="text-xs font-bold text-onyx select-none"
                    >
                      Set this as my default shipping address
                    </span>
                  </div>

                  <div className="flex gap-4 pt-4 border-t border-onyx/5">
                    <Button 
                      type="button" 
                      variant="outline" 
                      onClick={() => setIsAddingAddress(false)}
              ...
              {customer?.savedAddresses?.length > 0 ? (
                customer.savedAddresses.map((addr: any) => (
                  <Card 
                    key={addr._key} 
                    variant="outline" 
                    className={`bg-neutral-soft flex flex-col justify-between p-8 border-onyx/10 transition-all duration-300 relative group overflow-hidden ${
                      addr.isDefault ? 'border-2 border-onyx shadow-tactile' : 'hover:border-onyx'
                    }`}
                  >
                    <div className="space-y-2 mb-8">
                      <div className="flex justify-between items-start gap-4">
                        {addr.isDefault ? (
                          <span className="inline-block bg-onyx text-white text-[8px] uppercase tracking-widest font-black px-3 py-1 rounded-full">
                            Default Address
                          </span>
                        ) : (
                          <button
                            onClick={() => handleSetDefaultAddress(addr._key)}
                            className="text-[8px] font-black uppercase tracking-widest text-onyx/40 hover:text-onyx border-none bg-transparent p-0 cursor-pointer underline underline-offset-2"
                          >
                            Set Default
                          </button>
                        )}
                      ...
                      <p className="font-bold text-onyx pt-2 text-sm leading-tight">{addr.street}</p>
                      <p className="text-xs text-onyx/60 font-medium">
                        {addr.city}, {addr.state} {addr.zipCode}
                      </p>
                      <p className="text-xs text-onyx/40 font-medium">{addr.country}</p>
                    </div>
                ...
                <div className="col-span-full py-16 text-center text-onyx/40 font-medium italic text-sm border-2 border-dashed border-onyx/10 rounded-2xl bg-neutral-soft">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-8 animate-fade-in max-w-xl">
            <div>
              <h2 className="text-2xl font-black uppercase tracking-tight text-onyx">Profile Settings</h2>
              <p className="text-onyx/60 font-medium mt-1">Manage your public credentials and contact details.</p>
            </div>

            <Card variant="outline" className="bg-neutral-soft border-onyx/5 shadow-sm p-8 md:p-10">
              <form onSubmit={handleProfileUpdate} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Input 
                    ...
                    disabled
                    className="bg-bone cursor-not-allowed opacity-60 text-onyx/60"
                  />
                  <p className="text-[9px] text-onyx/40 font-medium ml-1">
                ...
                <div className="pt-6 border-t border-onyx/5">
                  <Button 
                    type="submit" 
